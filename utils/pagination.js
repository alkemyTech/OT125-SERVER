...
const getPreviousPage = (count, offset, limit) => {
    ...
    return count > offset ? Math.round(offset / limit) : Math.ceil(count / limit);
};

const getNextPage = (count, offset, limit) => {
    const totalPages = Math.ceil(count / limit);
    const nextPage = Math.round((offset + limit) / limit);
    return totalPages - nextPage > 0 ? parseInt(nextPage) + 1 : null;
};
...
module.exports = {
    paginate: async (repository, req, limit) => {
        const validPage = getPageValidate(req.query);

        if (!validPage){
            throw errorMessage(req.query.page, "Invalid parameter");
        }
        const offset = getOffset(req.query, limit);
        const { count, rows } = await repository(offset, limit);
    
        if (rows.length === 0 && offset !== 0){
            throw errorMessage(req.query.page, "This page does not exist");
        }
